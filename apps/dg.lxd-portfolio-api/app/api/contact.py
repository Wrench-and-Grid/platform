...
from app.core.rate_limit import _client_ip, rate_limit_contact
...
def _mask_email(email: str) -> str:
    """Return a partially redacted email safe to write to log files."""
    at = email.find("@")
    if at <= 0:
        return "***"
...
async def submit_contact(
    ...
    logger.info("Contact submission from %s — name=%r email=%s", ip, payload.name, _mask_email(str(payload.email)))
