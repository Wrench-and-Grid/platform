...
// ── Simulation palette (brand colours, dimmed for WebGL) ──────────────────────
...
export default function FluidCanvas({ className, interactionRegion }: FluidCanvasProps) {
  ...
  useEffect(() => {
    ...
    const generateColor = () => {
      ...
      palIdx++;
      return { r: c.r * PALETTE_SCALE, g: c.g * PALETTE_SCALE, b: c.b * PALETTE_SCALE };
    };
    ...
    const copyProg = createProgram(VERT_BASE, FRAG_COPY);
    const clearProg = createProgram(VERT_BASE, FRAG_CLEAR);
    const splatProg = createProgram(VERT_BASE, FRAG_SPLAT);
    const advProg = createProgram(
      VERT_BASE,
      FRAG_ADVECTION,
      supportLinearFiltering ? null : ["MANUAL_FILTERING"]
    );
    const divProg = createProgram(VERT_BASE, FRAG_DIVERGENCE);
    const curlProg = createProgram(VERT_BASE, FRAG_CURL);
    const vortProg = createProgram(VERT_BASE, FRAG_VORTICITY);
    const presProg = createProgram(VERT_BASE, FRAG_PRESSURE);
    const gradProg = createProgram(VERT_BASE, FRAG_GRAD_SUB);
    const dispMat = createMaterial(VERT_BASE, FRAG_DISPLAY);
